...
var DocumentBuilder = class {
  constructor() {
    ...
    this.__builder = new XMLBuilder(builderOptions);
  }
  /***
...
var InvoiceTypeCodeSchema = z.number().int().refine((code) => isValidInvoiceTypeCode(code), {
...
var EASCodeSchema = z2.enum(easCodeKeys);
...
var CountryCodeSchema = z3.enum(countryCodeKeys);

// src/documents/common/AdressSchema.ts
import { z as z4 } from "zod";
var addressSchema = z4.object({
  streetName: z4.string().optional(),
  additionalStreetName: z4.string().optional(),
  cityName: z4.string().optional(),
  postalZone: z4.string().optional(),
  countrySubentity: z4.string().optional(),
  addressLine: z4.string().optional(),
...
var CurrencyCodeSchema = z5.enum(currencyCodeKeys);
...
var UnitCodeSchema = z6.enum(unitCodeKeys);
...
var PaymentMeansCodeSchema = z7.enum(paymentMeansCodeKeys).describe("UNCL4461 Payment means code");
...
var DutyTaxFeeCategoryCodeSchema = z8.enum(dutyTaxFeeCategoryKeys);
var taxCategorySchema = z8.object({
  categoryCode: DutyTaxFeeCategoryCodeSchema,
  percent: z8.number().min(0).max(100).optional(),
  exemptionReason: z8.string().min(1).optional(),
  exemptionReasonCode: z8.string().min(1).optional()
});
...
var contactSchema = z9.object({
  name: z9.string().min(1).optional(),
  phone: z9.string().min(1).optional(),
  email: z9.string().min(1).optional()
});

// src/documents/common/Parties.ts
var partySchema = z10.object({
  endPoint: z10.object({
    scheme: EASCodeSchema,
    id: z10.string().min(1)
  }),
  identification: z10.object({
    scheme: EASCodeSchema.optional(),
    id: z10.string().min(1)
  }).array().optional(),
  name: z10.string().min(1).optional(),
  address: addressSchema,
  taxSchemeCompanyID: z10.string().min(1).optional(),
  legalEntity: z10.object({
    registrationName: z10.string().min(1),
    legalForm: z10.string().min(1).optional(),
...
var date = z11.union([z11.string(), z11.date()]);

// src/documents/common/LegalMonetaryTotal.ts
import { z as z12 } from "zod";
var legalMonetaryTotalSchema = z12.object({
  // Not in the peppol specification, but we make it mandatory here for flexibility
  currency: CurrencyCodeSchema,
  lineExtensionAmount: z12.number(),
  taxExclusiveAmount: z12.number(),
  taxInclusiveAmount: z12.number(),
  payableAmount: z12.number(),
  payableRoundingAmount: z12.number().optional(),
  prepaidAmount: z12.number().optional(),
  allowanceTotalAmount: z12.number().optional(),
  chargeTotalAmount: z12.number().optional(),
...
var taxSubtotalSchema = z13.object({
  taxableAmount: z13.number().min(0),
  taxAmount: z13.number().min(0),
...
var taxTotalSchema = z13.object({
  taxAmount: z13.number().min(0),
  taxAmountCurrency: CurrencyCodeSchema,
  subTotals: z13.array(taxSubtotalSchema)
});

// src/documents/common/Line.ts
import { z as z14 } from "zod";
var lineSchema = z14.object({
  id: z14.string().min(1),
  note: z14.string().optional(),
  invoicedQuantity: z14.number().min(0),
  lineExtensionAmount: z14.number().min(0),
  accountingCost: z14.string().optional(),
  invoicePeriod: z14.object({
    startDate: date.optional(),
    endDate: date.optional()
  }).optional(),
  orderLineReference: z14.string().optional(),
  documentReference: z14.string().optional(),
  //TODO: add AllowanceCharge
  name: z14.string(),
  description: z14.string().optional(),
  buyersItemIdentification: z14.string().optional(),
  sellersItemIdentification: z14.string().optional(),
  standardItemIdentification: z14.string().optional(),
  ...
  additionalItemProperties: z14.record(z14.string(), z14.any()).optional(),
  price: z14.number().min(0),
  currency: CurrencyCodeSchema,
...
var invoicePeriodSchema = z15.object({
  startDate: date.optional(),
  endDate: date.optional(),
  descriptionCode: z15.enum(["3", "35", "432"])
});
...
var financialAccountSchema = z16.object({
  id: z16.string().describe("Payment account identifier like IBAN or BBAN"),
  name: z16.string().optional().describe("Name of the account to which the payment should be made"),
  financialInstitutionBranch: z16.string().optional().describe("BIC or other clearing code")
});

// src/documents/common/PaymentMeans.ts
var paymentMeansSchema = z17.object({
  code: PaymentMeansCodeSchema,
  name: z17.string().optional().describe("Payment means expressed as text"),
  paymentId: z17.string().optional().describe("Used for reconciliation"),
...
// src/documents/invoices/Invoice.ts
var invoiceSchema = z18.object({
  customizationID: z18.string().optional(),
  profileID: z18.string().optional(),
  ID: z18.string().min(1),
  issueDate: date,
  ...
  accountingCost: z18.string().optional(),
  buyerReference: z18.string().optional(),
  invoicePeriod: invoicePeriodSchema.optional(),
  seller: partySchema,
  buyer: partySchema,
  paymentMeans: paymentMeansSchema.array().optional(),
  paymentTermsNote: z18.string().optional().describe("Payment terms that apply (including penalties)"),
  taxTotal: z18.array(taxTotalSchema).min(1).max(2),
  legalMonetaryTotal: legalMonetaryTotalSchema,
  invoiceLines: z18.array(lineSchema).min(1)
});
...
var CreditNoteTypeCodeSchema = z19.number().int().refine((code) => isValidCreditNoteTypeCode(code), {
  message: "Invalid CreditNoteTypeCode"
});

// src/documents/invoices/CreditNote.ts
var billingReference = z20.object({
  invoiceDocReference: z20.object({
    id: z20.string(),
    issueDate: date.optional()
  })
}).optional();
var creditNoteSchema = invoiceSchema.extend({
  customizationID: z20.string().optional(),
  profileID: z20.string().optional(),
...
var DocumentParser = class {
  constructor() {
    this.__parser = new XMLParser(parserOptions);
  }
  /***
...
var computeTotals = (items) => {
  const itemsWithLineExtensionAmount = items.map((item) => ({
    ...item,
    lineExtensionAmount: new Decimal(item.price).mul(new Decimal(item.quantity)).toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
  }));
  ...
  for (const [rate, taxableAmount] of baseAmountPerRate) {
    const groupTax = taxableAmount.mul(rate).div(100).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
...
export {
  ...
  basicCreditNote as exampleCreditNote,
  basicInvoice as exampleInvoice,
