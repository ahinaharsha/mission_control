    ...
    <div style={styles.page}>
      ...
      <nav style={styles.nav}>
        ...
        <div style={styles.navButtons}>
          <button style={styles.navBtn} onClick={() => onNavigate('retrieve')}>Retrieve</button>
          <button style={styles.navBtn} onClick={() => onNavigate('login')}>Login</button>
          <button style={{ ...styles.navBtn, ...styles.navBtnPrimary }} onClick={() => onNavigate('register')}>Register</button>
        </div>
      </nav>
    </div>
  );
...
const styles = {
  ...
  canvas: {
    ...
    pointerEvents: 'none',
    zIndex: 0,
    filter: 'blur(32px) brightness(0.9)', // ← blurred at the element level, no per-band cost
    opacity: 0.85,
  },
  nav: {
    position: 'relative',
